...
class MainPresenter(private val view: MainContract.View, private val workerRepository: WorkerRepository, private val context: Context) : MainContract.Presenter {
    override fun checkSign(uid: String) {
        val disposable = workerRepository.CheckSign(uid).observeOn(AndroidSchedulers.mainThread()).subscribeOn(Schedulers.io()).subscribeWith(object : DisposableObserver<ResponseResult<Boolean>>() {
