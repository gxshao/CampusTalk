...
class ChatActivity : Activity(), ChatContract.View, OnAudioRecoredStatusListener {
    ...
    class ChatHandler(activity: ChatActivity) : Handler() {
        private val mChatHand: ChatActivity by lazy {
            activity
        }

        override fun handleMessage(msg: Message) {
            val activity = mChatHand
            when (msg.what) {
                1 -> {
                    //匹配成功
...
//                       mAdapter.addOtherMsg(x,ChatAdapter.MSG_TYPE_IMGAE)
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        chatpresenter = ChatPrensenter(this, WorkerRepository.getInstance(WorkerRemoteDataSource.getInstance()), applicationContext)
    ...
    override fun initViews() {
        ...
        val bg_text = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            ...
            resources.getDrawable(R.drawable.btn_switch_text_bg)
        }
        ...
        btn_audio.setOnClickListener {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M
                ...
                this.requestPermissions(kotlin.arrayOf(Manifest.permission.RECORD_AUDIO), 2)
                return@setOnClickListener
            }
        }
